...
const fs = require('fs'),
	...
	DISTRO_JSON_PATH = './distro.json',
	ROOT_MODULES_FOLDER = './Modules/',
	NATIVE_MODULES_FOLDER = './Modules/suitecommerce/';
...
let suiteHelper = {
	...
	getCustomizationModuleName: function(fullPath, withoutVersion) {
		...
		if (withoutVersion) {
			return config.customModuleFolderAfix.prefix + fullPath.split('/')[0].replace(/[@]\d\.\d\.\d/, '') + config.customModuleFolderAfix.suffix;

		} else {
			let customModuleName = fullPath.split('/')[0].split('@');
		...
		if (!fs.existsSync(customizationFilePath)) {
			mkdirp(customizationFilePath, (err) => {
				if(err) {
					...
					let customizationNsPackagePath = ROOT_MODULES_FOLDER + config.customFolderName + '/' + customizationModuleName + '/' + 'ns.package.json';
					if (!fs.existsSync(customizationNsPackagePath)) {
						let stream = fs.createReadStream(NATIVE_MODULES_FOLDER + self.getModuleName(filePath) + '/' + 'ns.package.json').pipe(fs.createWriteStream(customizationNsPackagePath));
	...
	generateCustomizationFilePath: function(filePath) {
		...
		return ROOT_MODULES_FOLDER + config.customFolderName + '/' + customizationModuleName + '/' + fileType + '/';
	},
	...
	addCustModuleToDistro: function(distroJson, customizationModuleName) {
		distroJson.modules[config.customFolderName + '/' + customizationModuleName.replace('@' + config.customModuleVersion, '')] = config.customModuleVersion
	},
	...
	isSuiteScriptCustomization: function(filePath) {
		return filePath.toLowerCase().indexOf('/suitescript/') > -1;
	},
	...
	tplCustomizationHandler: function(filePath) {
		let packageJsonPath = ROOT_MODULES_FOLDER + config.customFolderName + '/' + this.getCustomizationModuleName(filePath) + '/' + 'ns.package.json';
		jsonfile.readFile(packageJsonPath, (err, nsPackageJson) => {

			if(!nsPackageJson.overrides){ nsPackageJson.overrides = {}}
			nsPackageJson.overrides['suitecommerce/' + filePath] = "Templates/" + this.getFileName(filePath);
	...
	suiteScriptCustomizationHandler: function(distroJson, filePath, customizationFileName) {
		...
		if (this.isBackendConfiguration(filePath)) {
			...
			jsonfile.readFile(packageJsonPath, (err, nsPackageJson) => {
				...
				if(!nsPackageJson.overrides){ nsPackageJson.overrides = {}}
				nsPackageJson.overrides['suitecommerce/' + filePath] = "SuiteScript/" + this.getFileName(filePath);
	...
	loadConfigFile: function(callback) {
		...
		configFilePath = configFilePath + '/' + CONFIG_FILE_NAME;
